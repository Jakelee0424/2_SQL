...
/*  서브쿼리 유형
    ...
    - 단일행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개(1행)일 때 
    
    - 다중행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개(여러 행)일 때
...
FROM EMPLOYEE 
JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE (JOB_CODE, TRUNC(SALARY,-4)) IN (SELECT JOB_CODE, TRUNC(AVG(SALARY),-4) --      '= ANY' : IN이랑 같은 기능
										FROM EMPLOYEE 
										GROUP BY JOB_CODE);
